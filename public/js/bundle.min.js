...
var Sentry = (function (t) {
			...
			return (function (t) {
				...
				(n.reverse(),
					z.test(n[n.length - 1].function || "") &&
						(n.pop(),
						z.test(n[n.length - 1].function || "") && n.pop()));
				return n
					.slice(0, 50)
					.map((t) => ({
						...t,
	...
	function et(t) {
		if (!nt[t])
			switch (((nt[t] = !0), t)) {
				case "console":
					!(function () {
						if (!("console" in $)) return;
						O.forEach(function (t) {
							t in $.console &&
								C($.console, t, function (n) {
									return (
										(st[t] = n),
										function (...n) {
											it("console", {
	...
	function ht(t, n = !1) {
		return (e) => {
			...
			((void 0 === at ||
				...
				(t({ event: e, name: r, global: n }), (ft = e)),
				clearTimeout(at),
				(at = Z.setTimeout(() => {
					at = void 0;
				}, 1e3)));
	...
	class kt {
		...
		__init() {
			this.u = (t) => {
				this.p(St.RESOLVED, t);
			};
		}
		__init2() {
			this.h = (t) => {
				this.p(St.REJECTED, t);
			};
		}
		__init3() {
			this.p = (t, n) => {
		...
		__init4() {
			this.l = () => {
				if (this.i === St.PENDING) return;
				const t = this.o.slice();
				((this.o = []),
					t.forEach((t) => {
						t[0] ||
							(this.i === St.RESOLVED && t[1](this.m),
							this.i === St.REJECTED && t[2](this.m),
							(t[0] = !0));
					}));
			};
		}
	...
	function Ht(t, n) {
		...
		function s(t) {
			"string" == typeof i
				? (i = "string" == typeof t ? i + t : [qt(i, n), t])
	...
	function Bt(t, n) {
		const e = "string" == typeof t.data ? qt(t.data, n) : t.data;
		return [
			B({
				type: "attachment",
				length: e.length,
	...
	function Wt(t, { statusCode: n, headers: e }, r = Date.now()) {
		const i = { ...t },
	...
	function Kt(t, n = {}) {
		if (
			...
			(t.timestamp = n.timestamp || Lt()),
			n.ignoreDuration && (t.ignoreDuration = n.ignoreDuration),
			n.sid && (t.sid = 32 === n.sid.length ? n.sid : pt()),
	...
	class Yt {
		...
		setContext(t, n) {
			return (
				null === n ? delete this.D[t] : (this.D[t] = n),
				this.P(),
				this
			);
		}
		...
		H(t, n, e, r = 0) {
			return new kt((i, s) => {
				...
				if (null === n || "function" != typeof o) i(n);
					...
					l(c)
						? c
								.then((n) => this.H(t, n, e, r + 1).then(i))
								.then(null, s)
						: this.H(t, c, e, r + 1)
								.then(i)
								.then(null, s);
		...
		q(t) {
			((t.fingerprint = t.fingerprint ? _t(t.fingerprint) : []),
	...
	function Zt() {
		return { traceId: pt(), spanId: pt().substring(16) };
	}
	const tn = 100;
	class nn {
		constructor(t, n = new Yt(), e = 4) {
			((this.B = e), (this.F = [{ scope: n }]), t && this.bindClient(t));
		}
		isOlderThan(t) {
			return this.B < t;
		}
		...
		pushScope() {
			const t = Yt.clone(this.getScope());
			return (
		...
		captureException(t, n) {
			const e = (this.X = n && n.event_id ? n.event_id : pt()),
		...
		captureMessage(t, n, e) {
			const r = (this.X = e && e.event_id ? e.event_id : pt()),
				i = new Error(t);
			return (
				this.W((s, o) => {
					s.captureMessage(
		...
		captureEvent(t, n) {
			const e = n && n.event_id ? n.event_id : pt();
			return (
				t.type || (this.X = e),
				this.W((r, i) => {
		...
		addBreadcrumb(t, n) {
			...
			if (!r) return;
			const { beforeBreadcrumb: i = null, maxBreadcrumbs: s = tn } =
				(r.getOptions && r.getOptions()) || {};
			if (s <= 0) return;
			const o = { timestamp: Ct(), ...t },
				c = i
					? (function (t) {
					...
					: o;
			null !== c &&
				(r.emit && r.emit("beforeAddBreadcrumb", c, n),
				e.addBreadcrumb(c, s));
		}
		setUser(t) {
		...
		startSession(t) {
			...
			return (
				c && "ok" === c.status && Kt(c, { status: "exited" }),
				this.endSession(),
				n.setSession(o),
				o
			);
	...
	function en() {
		return (
			($.__SENTRY__ = $.__SENTRY__ || { extensions: {}, hub: void 0 }),
			$
		);
	}
	function rn(t) {
	...
	function captureException(t, n) {
		return sn().captureException(t, { captureContext: n });
	}
	function un(t) {
		sn().withScope(t);
	}
	...
	function hn(t, n = {}) {
		...
		return (
			e ||
			`${(function (t) {
				return `${an(t)}${t.projectId}/envelope/`;
			})(t)}?${fn(t, r)}`
		);
	}
	function ln(t, n, e, r) {
	...
	function yn(t, n) {
		((n[t.name] = t),
			-1 === dn.indexOf(t.name) &&
				(t.setupOnce(Qt, sn), dn.push(t.name)));
	}
	function mn(t, n, e, r) {
		const { normalizeDepth: i = 3, normalizeMaxBreadth: s = 1e3 } = t,
			o = {
				...n,
				event_id: n.event_id || e.event_id || pt(),
				timestamp: n.timestamp || Ct(),
			},
		...
		(!(function (t, n) {
			...
			"environment" in t || (t.environment = "environment" in n ? e : Gt);
		...
		let u = r;
		e.captureContext && (u = Yt.clone(u).update(e.captureContext));
		let a = xt(o);
		if (u) {
			if (u.getAttachments) {
				...
				"number" == typeof i && i > 0
					? (function (t, n, e) {
							...
							const r = {
								...
								...(t.user && { user: wt(t.user, n, e) }),
								...(t.contexts && {
									contexts: wt(t.contexts, n, e),
								}),
								...(t.extra && { extra: wt(t.extra, n, e) }),
							};
							t.contexts &&
								t.contexts.trace &&
								r.contexts &&
	...
	function _n(t, n, e = jt(t.bufferSize || 30)) {
		let r = {};
		function i(i) {
			const s = [];
			if (
			...
			return e
				...
				.then(
					(t) => t,
					(t) => {
						if (t instanceof A) return (c("queue_overflow"), xt());
						throw t;
					},
				);
		}
	...
	class kn {
		...
		constructor(t = {}) {
			((this.name = kn.id), (this.Y = t));
		}
		setupOnce(t, n) {
			const e = (t) => {
				const e = n();
				if (e) {
					const n = e.getIntegration(kn);
					if (n) {
						const r = e.getClient(),
							i = r ? r.getOptions() : {},
							s = (function (t = {}, n = {}) {
								return {
									...
									ignoreErrors: [
										...(t.ignoreErrors || []),
										...(n.ignoreErrors || []),
										...(t.disableErrorDefaults ? [] : xn),
									],
									ignoreTransactions: [
										...(t.ignoreTransactions || []),
										...(n.ignoreTransactions || []),
										...(t.disableTransactionDefaults
											? []
											: Tn),
									],
									ignoreInternal:
										void 0 === t.ignoreInternal ||
										t.ignoreInternal,
								};
							})(n.Y, i);
						return (function (t, n) {
				...
				throw (
					...
					un((e) => {
						(e.addEventProcessor(
							(t) => (
								n.mechanism &&
									(vt(t, void 0, void 0), gt(t, n.mechanism)),
	...
	function Ln(t, n) {
		return { exception: { values: [Cn(t, n)] } };
	}
	function Nn(t, n) {
	...
	const Un = /Minified React error #\d+;/i;
	function Pn(t) {
		const n = t && t.message;
		return n
	...
	function Hn(t, n, e = "info", r, i) {
		const s = Fn(t, n, (r && r.syntheticException) || void 0, i);
		return (
			(s.level = e),
	...
	function Bn(t, n, e, r, u) {
		...
		if (i(n)) return Ln(t, n);
		if (f(n) || h(n)) {
			return (
				(a = (function (t, n, e, r) {
					const i = sn().getClient(),
						...
						o = {
							...
							extra: { __serialized__: Et(n, s) },
						};
					if (e) {
						const n = Nn(t, e);
						n.length &&
							(o.exception.values[0].stacktrace = { frames: n });
					}
					return o;
				})(t, n, e, u)),
				gt(a, { synthetic: !0 }),
	...
	function Fn(t, n, e, r) {
		...
		if (r && e) {
			...
			r.length &&
				(i.exception = {
					values: [{ value: n, stacktrace: { frames: r } }],
				});
		}
		return i;
	}
	function Xn(t, { isUnhandledRejection: n }) {
	...
	function zn(t, { metadata: n, tunnel: e, dsn: r }) {
		const i = {
				event_id: t.event_id,
				sent_at: new Date().toISOString(),
				...(n &&
	...
	class Wn extends class {
		...
		captureEvent(t, n, e) {
			if (n && n.originalException && bt(n.originalException)) return;
			let r = n && n.event_id;
			return (
				this.it(
		...
		captureSession(t) {
			this.ot() &&
				("string" != typeof t.release ||
					(this.sendSession(t), Kt(t, { init: !1 })));
		}
		getDsn() {
			return this.et;
		}
		...
		flush(t) {
			const n = this.rt;
			return n
				? this.ct(t).then((e) => n.flush(t).then((t) => e && t))
				: xt(!0);
		}
		close(t) {
		...
		sendEvent(t, n = {}) {
			if ((this.emit("beforeSendEvent", t, n), this.et)) {
				let e = ln(t, this.et, this.Y.K, this.Y.tunnel);
				for (const t of n.attachments || [])
					e = Ut(
						e,
						Bt(
							t,
							this.Y.transportOptions &&
								this.Y.transportOptions.textEncoder,
						),
					);
				const r = this.ut(e);
		...
		lt(t, n, e) {
			...
			if (!this.ot())
				return Tt(
					new A("SDK not enabled, will not capture event.", "log"),
				);
			const s = bn(t),
				o = gn(t),
				c = t.type || "error",
			...
			return this.ht(t, n, e)
				.then((e) => {
					...
					const i = (function (t, n, e) {
						...
						if (gn(n) && r) return r(n, e);
						if (bn(n) && i) return i(n, e);
						return n;
					})(r, e, n);
					return (function (t, n) {
		...
		constructor(t) {
			const n = Dn.SENTRY_SDK_SOURCE || "npm";
			((t.K = t.K || {}),
				(t.K.sdk = t.K.sdk || {
					name: "sentry.javascript.browser",
					packages: [{ name: `${n}:@sentry/browser`, version: En }],
					version: En,
				}),
				super(t),
				t.sendClientReports &&
					Dn.document &&
					Dn.document.addEventListener("visibilitychange", () => {
						"hidden" === Dn.document.visibilityState && this.yt();
					}));
		}
		eventFromException(t, n) {
			return qn(this.Y.stackParser, t, n, this.Y.attachStacktrace);
		}
		eventFromMessage(t, n = "info", e) {
			return Hn(this.Y.stackParser, t, n, e, this.Y.attachStacktrace);
		}
		...
		yt() {
			...
			const n =
				((e = t),
				Nt((r = this.Y.tunnel && R(this.et)) ? { dsn: r } : {}, [
					[
						{ type: "client_report" },
						{ timestamp: i || Ct(), discarded_events: e },
					],
				]));
			var e, r, i;
			this.ut(n);
	...
	function Jn(
		...
		n = (function () {
			if (Gn) return Gn;
			if (V(Dn.fetch)) return (Gn = Dn.fetch.bind(Dn));
			const t = Dn.document;
			let n = Dn.fetch;
	...
	const Qn =
		...
		ee = /(\S+) line (\d+)(?: > eval line \d+)* > eval/i,
		re = [
			50,
			(t) => {
				const n = ne.exec(t);
				if (n) {
					...
					return (
						([e, t] = le(e, t)),
						Vn(t, e, n[4] ? +n[4] : void 0, n[5] ? +n[5] : void 0)
					);
				}
			},
		],
		...
		se = [
			...
			(t) => {
				const n = ie.exec(t);
				return n
					? Vn(n[2], n[1] || Yn, +n[3], n[4] ? +n[4] : void 0)
					: void 0;
			},
		],
		oe = / line (\d+).*script (?:in )?(\S+)(?:: in function (\S+))?$/i,
		ce = [
			10,
			(t) => {
				const n = oe.exec(t);
				return n ? Vn(n[2], n[3] || Yn, +n[1]) : void 0;
			},
		],
		ue =
		...
		ae = [
			...
			(t) => {
				const n = ue.exec(t);
				return n ? Vn(n[5], n[3] || n[4] || Yn, +n[1], +n[2]) : void 0;
			},
		],
		fe = [te, re, se],
		he = W(...fe),
	...
	class de {
		...
		constructor(t) {
			((this.name = de.id),
				...
				(this.vt = { onerror: pe, onunhandledrejection: ye }));
		}
		setupOnce() {
	...
	function pe() {
		rt("error", (t) => {
			const [n, e, r] = ge();
			if (!n.getIntegration(de)) return;
			const { msg: i, url: s, line: c, column: a, error: f } = t;
			if (In() || (f && f.__sentry_own_request__)) return;
			const h =
				void 0 === f && u(i)
					? (function (t, n, e, r) {
							const i =
					...
					: me(Bn(e, f || i, void 0, r, !1), s, c, a);
			((h.level = "error"), ve(n, f, h, "onerror"));
		});
	}
	function ye() {
		rt("unhandledrejection", (t) => {
			...
			if (In() || (i && i.__sentry_own_request__)) return !0;
			const s = a(i)
				? {
						exception: {
							values: [
				...
				: Bn(e, i, void 0, r, !0);
			((s.level = "error"), ve(n, i, s, "onunhandledrejection"));
		});
	}
	function me(t, n, e, r) {
	...
	class _e {
		...
		setupOnce() {
			(this.Y.setTimeout && C(Dn, "setTimeout", we),
				this.Y.setInterval && C(Dn, "setInterval", we),
				this.Y.requestAnimationFrame &&
					C(Dn, "requestAnimationFrame", Ee),
				this.Y.XMLHttpRequest &&
					"XMLHttpRequest" in Dn &&
					C(XMLHttpRequest.prototype, "send", $e));
			const t = this.Y.eventTarget;
			if (t) {
				(Array.isArray(t) ? t : be).forEach(Se);
			}
		}
	}
	function we(t) {
					...
					t in e &&
						...
						C(e, t, function (n) {
							const e = {
								...
								r = U(n);
							return (
								r && (e.mechanism.data.handler = J(r)),
								An(n, e)
							);
						});
				}),
				...
				return function (e, r, i) {
					try {
						"function" == typeof r.handleEvent &&
							(r.handleEvent = An(r.handleEvent, {
								mechanism: {
									data: {
	...
	class Te {
		...
		setupOnce() {
			if (
				(this.options.console && rt("console", je),
				this.options.dom &&
					rt(
						"dom",
						(function (t) {
				...
				this.options.xhr && rt("xhr", Oe),
				this.options.fetch && rt("fetch", De),
				this.options.history && rt("history", Re),
				this.options.sentry)
			) {
				const t = sn().getClient();
				t && t.on && t.on("beforeSendEvent", ke);
			}
		}
	}
	function ke(t) {
	...
	function je(t) {
		...
		const n = {
			...
			level:
				((e = t.level),
				"warn" === e ? "warning" : Dt.includes(e) ? e : "log"),
			message: y(t.args, " "),
		};
		var e;
		if ("assert" === t.level) {
		...
		sn().addBreadcrumb(n, { input: t.args, level: t.level });
	}
	function Oe(t) {
		...
		sn().addBreadcrumb({ category: "xhr", data: u, type: "http" }, a);
	}
	function De(t) {
		const { startTimestamp: n, endTimestamp: e } = t;
		if (
			...
			if (t.error) {
				...
				sn().addBreadcrumb(
					{ category: "fetch", data: r, type: "http" },
					i,
				);
			}
	}
	...
	class Ie {
		...
		setupOnce(t, n) {
			t((t, e) => {
				const r = n(),
					i = r.getClient(),
					s = r.getIntegration(Ie);
				if (!i || !s) return t;
				const o = i.getOptions();
				return (
					g(Cn, o.stackParser, o.maxValueLength, s.gt, s.bt, t, e),
					t
				);
			});
		}
	...
	let Be = {};
	Dn.Sentry && Dn.Sentry.Integrations && (Be = Dn.Sentry.Integrations);
	const Fe = { ...Be, ...On, ...Pe };
	return (
		(Fe.Replay = n),
		(Fe.BrowserTracing = e),
		...
		(t.captureMessage = function (t, n) {
			...
			return sn().captureMessage(t, e, r);
		}),
		(t.captureUserFeedback = function (t) {
			const n = sn().getClient();
			n && n.captureUserFeedback(t);
		}),
		(t.chromeStackLineParser = te),
		...
		(t.createUserFeedbackEnvelope = zn),
		(t.defaultIntegrations = qe),
		(t.defaultStackLineParsers = fe),
		(t.defaultStackParser = he),
		(t.eventFromException = qn),
		(t.eventFromMessage = Hn),
		(t.exceptionFromError = Cn),
		(t.flush = async function (t) {
			const n = sn().getClient();
			return n ? n.flush(t) : Promise.resolve(!1);
		}),
		(t.forceLoad = function () {}),
		(t.geckoStackLineParser = re),
		(t.getCurrentHub = sn),
		(t.getHubFromCarrier = on),
		(t.init = function (t = {}) {
			(void 0 === t.defaultIntegrations && (t.defaultIntegrations = qe),
				void 0 === t.release &&
					...
					Dn.SENTRY_RELEASE &&
						Dn.SENTRY_RELEASE.id &&
						(t.release = Dn.SENTRY_RELEASE.id)),
			...
			const n = {
				...t,
				stackParser:
					((e = t.stackParser || he), Array.isArray(e) ? W(...e) : e),
				integrations: pn(t),
				transport: t.transport || (Y() ? Jn : Kn),
			};
			var e;
			(!(function (t, n) {
				!0 === n.debug &&
		...
		(t.makeMain = rn),
		(t.makeXHRTransport = Kn),
		(t.onLoad = function (t) {
			t();
		}),
		(t.opera10StackLineParser = ce),
		(t.opera11StackLineParser = ae),
		...
		(t.showReportDialog = function (t = {}, n = sn()) {
			if (!Dn.document) return;
			const { client: e, scope: r } = n.getStackTop(),
				i = t.dsn || (e && e.getDsn());
			if (!i) return;
			(r && (t.user = { ...r.getUser(), ...t.user }),
				t.eventId || (t.eventId = n.lastEventId()));
			const s = Dn.document.createElement("script");
			((s.async = !0),
				(s.crossOrigin = "anonymous"),
				(s.src = (function (t, n) {
					const e = M(t);
					if (!e) return "";
					const r = `${an(e)}embed/error-page/`;
					let i = `dsn=${R(e)}`;
					for (const t in n)
						if ("dsn" !== t)
							if ("user" === t) {
								...
								if (!t) continue;
								(t.name &&
									(i += `&name=${encodeURIComponent(t.name)}`),
									t.email &&
			...
			const o = Dn.document.head || Dn.document.body;
			o && o.appendChild(s);
		}),
		(t.startTransaction = function (t, n) {
			return sn().startTransaction({ ...t }, n);
		}),
		(t.winjsStackLineParser = se),
		(t.withScope = un),
		(t.wrap = function (t) {
			return An(t)();
		}),
