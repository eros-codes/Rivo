...
function showError(input, message) {
	...
	err.style.cssText = "color:#e05c5c;font-size:0.75rem;margin-top:-0.5rem;display:block;";
...
if (signupForm) {
	signupForm.addEventListener("submit", (e) => {
		...
		if (!isValidUsername(signupUsername.value.trim())) {
			showError(signupUsername, "Username must be 3-20 characters, letters, numbers, or underscores only.");
			valid = false;
		} else {
	...
	codeDigits.forEach((input, idx) => {
		...
		input.addEventListener("keydown", (e) => {
			if (e.key === "Backspace" && !input.value && idx > 0) {
				...
				e.preventDefault();
				codeDigits[idx - 1].focus();
			} else if (e.key === "ArrowRight" && idx < codeDigits.length - 1) {
				e.preventDefault();
				codeDigits[idx + 1].focus();
			}
		});
		...
		input.addEventListener("paste", (e) => {
			...
			const focusIndex = Math.min(paste.length, codeDigits.length - 1);
...
if (forgotForm) {
	forgotForm.addEventListener("submit", function (e) {
		...
		if (!forgotInput.value.trim() || forgotInput.value.trim().length < 6) {
