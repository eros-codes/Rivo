...
// Module-level reference to the Socket.IO server instance. Prefer these
// helpers instead of relying on a global variable so route handlers can
// safely target sockets without reading `globalThis`.
let ioInstance = null;
...
export function initSocket(httpServer) {
	...
	// NOTE: exported from module top-level to allow background jobs to perform
	// O(1) lookups instead of scanning all sockets.
	...
	async function getRecipientsCached(convId) {
		...
		try {
			...
			// Evict oldest if recipients cache grows too large
			try {
				if (recipientsCache.size >= RECIPIENTS_CACHE_MAX) {
					const oldest = recipientsCache.keys().next().value;
					if (oldest) recipientsCache.delete(oldest);
				}
			} catch (e) { /* ignore eviction errors */ }
	...
	io.on("connection", async (socket) => {
			...
			try {
				...
				for (const c of contacts) {
					...
					for (const sid of sids) {
						...
						if (s) s.emit("user:online", { userId: socket.userId });
					}
				}
			} catch (e) {
					...
					try {
						...
						for (const uid of offlineTargetIds) {
							...
							push.sendNotificationToUser(uid, {
								title:
									message.isTimeCapsule && message.isLocked
										? "New message"
		...
		socket.on("message:delete", async (data, callback) => {
			...
			try {
				...
				// so contact previews stay in sync. Compute recipients once and reuse
				// to avoid redundant cached/DB calls.
				let allRecipients = null;
				try {
					if (!message.isSeen) {
						allRecipients = await getRecipientsCached(message.conversationId);
						const recipientContacts = allRecipients.filter((c) => c.ownerId !== socket.userId);
				...
				try {
					if (!allRecipients) allRecipients = await getRecipientsCached(message.conversationId);
					const recipientContacts = allRecipients.filter((c) => c.ownerId !== socket.userId);
					const usersInRoom = convoOnline.get(message.conversationId) || new Set();
					const recipientUserIds = Array.from(new Set(recipientContacts.map((c) => c.ownerId)));
					for (const uid of recipientUserIds) {
			...
			socket.on("reaction:add", async ({ messageId, emoji }, callback) => {
				try {
					...
					try {
						...
						console.error('failed to fetch recipients for reaction delivery', e);
						allRecipients = [];
					}
					try {
						const recipientContacts = allRecipients.filter((c) => c.ownerId !== socket.userId);
		...
		// ─── Disconnect ────────────────────────────────────────────────────────
			...
			try {
							...
							try {
								...
								if (!still || still.size === 0) {
									...
									try {
										...
										for (const c of contacts) {
											...
											for (const sid of sids) {
												const s = io.sockets.sockets.get(sid);
												if (s) s.emit("user:offline", { userId: socket.userId, lastSeen, privacyOnline: updated.privacyOnline });
											}
										}
									} catch (e) {
								...
								try {
									...
									for (const c of contacts) {
										...
										for (const sid of sids) {
											const s = io.sockets.sockets.get(sid);
											if (s) s.emit("user:offline", { userId: socket.userId, lastSeen, privacyOnline: null });
										}
									}
								} catch (_) { /* ignore */ }
							} finally {
				...
				try {
					if (socket._lastTyping && typeof socket._lastTyping.clear === 'function') {
	...
	// Keep a module-local reference for other modules to use via helpers.
	ioInstance = io;
