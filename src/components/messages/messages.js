...
export function createMessage({
	...
	${
		forwardedFrom
			? `
			<div class="chat-forwarded-label">Forwarded from ${escapeHtml(forwardedFrom)}</div>`
			: ""
	}
	...
	<span class="chat-message-meta">
		...
		${!user && isEdited ? `<span class="chat-edited-label">edited</span>` : ""}
		${user && isSeen ? `<span class="chat-message-status">${seenIcon}</span>` : ""}
		${user && !isSeen ? `<span class="chat-message-status">${sentIcon}</span>` : ""}
		${!user && isPinned ? `<span class="chat-pinned-icon">${pinIcon}</span>` : ""}
	</span>
	`;
